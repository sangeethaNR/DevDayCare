...
const uploadFood= async (event) =>{
  ...
  //  console.log("food:"+activityType +"desc" +desc)
    const {data} = await addFoodMutation({variables:{student_id:`${itemId}`,mealDesc:mealDesc,
  ...
  return (
    ...
    {/* End footer */}
         ...
         <TextField id="mealSession" label="mealSession" variant="outlined" value={mealSession}
onChange={(event)=> setMealSession(event.target.value)} />
<TextField id="activityDesc" label="Description" variant="outlined" value={mealDesc}
  onChange={(event)=> setMealDesc(event.target.value)} />
  <button style={{maxWidth:"6rem", height:"4rem", marginRight:"1rem"}} onClick={(event) =>{uploadFood(event)}}>Upload Meal</button>
