...
const Incidents = () => {
  ...
  const {  data } = useQuery(QUERY_INCIDENTREPORT,
...
const uploadIR= async (event) =>{
  ...
  console.log("desc" +desc)
    const {data} = await addIRMutation({variables:{student_id:`${itemId}`,desc:desc,
   }});
      ...
      <Container sx={{ py: 8 }} maxWidth="md">
        ...
        <Grid container spacing={4}>
          {incidentReports.map((incidentReport) => (
            <Grid item key={incidentReport._id} xs={12} sm={6} md={4}>
              <Card
...
<TextField id="incidentReport" label="Description" variant="outlined" value={desc}
  onChange={(event)=> setDesc(event.target.value)} />
  <button style={{maxWidth:"6rem", height:"4rem", marginRight:"1rem"}} onClick={(event) =>{uploadIR(event)}}>Upload Incidents</button>
