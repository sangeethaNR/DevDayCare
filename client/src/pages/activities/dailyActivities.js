...
const  DailyActivities = () =>{
  ...
  const {  data } = useQuery(QUERY_ACTIVITY,
...
// },[activityState])
    return (
      ...
      {/* End footer */}
           ...
           <TextField id="activityType" label="activityType" variant="outlined" value={activityType}
  onChange={(event)=> setActivityType(event.target.value)} />
<TextField id="activityDesc" label="Description" variant="outlined" value={desc}
    onChange={(event)=> setDesc(event.target.value)} />
    <button style={{maxWidth:"6rem", height:"4rem", marginRight:"1rem"}} onClick={(event) =>{uploadActivity(event)}}>Upload Activity</button>
